...
- (id)initWithFrame:(CGRect)frame style:(KalTileStyle)theStyle mode:(BOOL)theMode
{
    if ((self = [super initWithFrame:frame])) {
        ...
        for (int i=0; i<6; i++) {
            for (int j=0; j<7; j++) {
                CGRect r = CGRectMake(j*kTileSize.width*kScaleFactor, i*kTileSize.height*kScaleFactor, kTileSize.width*kScaleFactor, kTileSize.height*kScaleFactor);
                KalTileView *tileView = [[[KalTileView alloc] initWithFrame:r] autorelease];
                tileView.style = theStyle;
                tileView.isInDiyMode = theMode;
...
{
    self = [self initWithFrame:frame style:theStyle mode:YES];
    if (self) {
        //
    }
    return self;
...
- (void)sizeToFit
{
    self.height = kTileSize.height*kScaleFactor * numWeeks;
}
