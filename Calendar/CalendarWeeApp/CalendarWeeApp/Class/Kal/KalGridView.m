...
const CGSize kTileSize = { 38.f, 30.f };
...
{
    frame.size.width = 7 * kTileSize.width * kScaleFactor;
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        frontMonthView = [[KalMonthView alloc] initWithFrame:monthRect style:theStyle mode:theMode];
        backMonthView = [[KalMonthView alloc] initWithFrame:monthRect style:theStyle mode:theMode];
        ...
        recognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeGestureFrom:)];
...
{
    self = [self initWithFrame:frame logic:theLogic delegate:theDelegate style:theStyle mode:YES];
    if (self) {
        //
    }
    return self;
...
{
    self = [self initWithFrame:frame logic:theLogic delegate:theDelegate style:KalTileStyleSolar];
    if (self) {
        //
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([hitView isKindOfClass:[KalTileView class]]) {
        ...
        [delegate selectedDate:tile.date];
    }
    self.highlightedTile = nil;
}
#endif

- (void)swipeGestureFrom:(UISwipeGestureRecognizer *)recognizer
{
    if (transitioning) {
        return;
    }
...
- (void)swapMonthsAndSlide:(int)direction keepOneRow:(BOOL)keepOneRow
{
    ...
    if (direction == SLIDE_UP) {
        backMonthView.top = keepOneRow ? frontMonthView.bottom - kTileSize.height : frontMonthView.bottom;
...
//    self.selectedTile = [frontMonthView firstTileOfMonth];
    ...
    NSLog(@"keepOneRow:%d, self.frame:%@", keepOneRow, NSStringFromCGRect(self.frame));
}
