...
- (NSString *)stringForDisplay {
	...
	[self append:weiBoStr to:result];
	[self append:jobStr to:result];
	
	[self append:FAXStr to:result];
	[self append:ZIPCodeStr to:result];
...
- (void)populateActions {
    [actions addObject:[AddContactAction actionWithName:[self.names objectAtIndex:0]
                                           ...
                                           organization:self.organization
						                       ...
						                       DivWeiBo:self.weiBoStr 
												 TilJob:self.jobStr
						                        FaxNumber:self.FAXStr 
												ZipCode:self.ZIPCodeStr
						]];
}
...
- (UIImage *)icon {
    return [[UIImageManager shareInstance] imageWithFileName:@"TwoDimension/business-card.png"];
}
