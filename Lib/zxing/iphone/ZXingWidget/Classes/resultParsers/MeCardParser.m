...
+ (ParsedResult *)parsedResultForString:(NSString *)s
  ...
  NSRange foundRange1 = [s rangeOfString:@"MECARD:"];
  NSRange foundRange2 = [s rangeOfString:@"CARD:"];
  if (foundRange1.location == NSNotFound && foundRange2.location == NSNotFound) {
    return nil;
  }
  ...
  result.phoneNumbers = (NSMutableArray *)[[s fieldsWithPrefix:@"TEL:"] stringArrayWithTrimmedWhitespace];  
  NSString *phoneNumber= [[s fieldWithPrefix:@"M:"] stringWithTrimmedWhitespace];  
	if (result.phoneNumbers&&phoneNumber&&[phoneNumber isKindOfClass:[NSString class]]) {
		[result.phoneNumbers addObject:phoneNumber];
	}else{
		if (phoneNumber&&[phoneNumber isKindOfClass:[NSString class]]) {
			result.phoneNumbers = [NSMutableArray arrayWithObject:phoneNumber];
		}		
	}  result.emails = [[s fieldsWithPrefix:@"EMAIL:"] stringArrayWithTrimmedWhitespace];
  if (!result.emails) {
	result.emails = [[s fieldsWithPrefix:@"EM:"] stringArrayWithTrimmedWhitespace];
  }
  result.note = [s fieldWithPrefix:@"NOTE:"];
	if (!result.note) {//added by ygf 
		result.note = [s fieldWithPrefix:@"IM:"];
	}
  ...
  result.organization = [[s fieldWithPrefix:@"ORG:"] stringWithTrimmedWhitespace];
	if (!result.organization) {
		result.organization = [[s fieldWithPrefix:@"COR:"] stringWithTrimmedWhitespace];
	}
  result.jobTitle = [s fieldWithPrefix:@"TITLE:"];
	
	
  //added by ygf for more information
  result.weiBoStr = [s fieldWithPrefix:@"DIV:"];
  result.jobStr = [s fieldWithPrefix:@"TIL:"];
	
	
	//ZIP: FAX:  TEL:M: together
	result.FAXStr = [s fieldWithPrefix:@"FAX:"];
	result.ZIPCodeStr = [s fieldWithPrefix:@"ZIP:"];
