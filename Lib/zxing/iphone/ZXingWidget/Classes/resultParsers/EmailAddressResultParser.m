...
+ (ParsedResult *)parsedResultForString:(NSString *)rawText
    ...
    if (url && [[[url scheme] lowercaseString] isEqualToString:kMailto]) {
        ...
        if (!emailAddress) {
			...
			{
				mailAddress =
				[[rawText fieldWithPrefix:@"mailto:"] stringWithTrimmedWhitespace];
			}
			if (!mailAddress) {
				mailAddress =
				[[rawText fieldWithPrefix:@"MAILTo:"] stringWithTrimmedWhitespace];
			...
			if (!mailAddress) {
				NSArray *infoArr =[rawText componentsSeparatedByString:@":"];
				for (NSUInteger index = 0; index<[infoArr count]; index++) {
					infoItem = [infoArr objectAtIndex:index];
					if (infoItem&&[infoItem isKindOfClass:[NSString class]]) {
						if ([[infoItem lowercaseString]isEqualToString:kMailto]) {
							if ((index+1)<[infoArr count]) {
								mailAddress = [infoArr objectAtIndex:index+1];
								break;
							}
						}
					}
