...
- (NSArray *)fieldsWithPrefix:(NSString *)prefix terminator:(NSString *)term {
  ...
  while (i < max) {
    ...
    if(foundRange.location == NSNotFound) {
	  ...
	  //bug: MSN: is found for N:
	  if ([prefix isEqualToString:@"N:"]) {
		  if (foundRange.location>=2) {
			  NSString *subStr = [self substringWithRange:NSMakeRange(foundRange.location-2, foundRange.length+2)];
			  if ([subStr hasPrefix:@"MSN"]) {//MSN:
				  break;
			  }
		  }
	  }
	  if ([prefix isEqualToString:@"M:"]) {
		  if (foundRange.location>=1) {
			  NSString *subStr = [self substringWithRange:NSMakeRange(foundRange.location-1, foundRange.length+1)];
			  if ([subStr hasPrefix:@"IM"] || [subStr hasPrefix:@"EM"]) {//IM:
				  break;
			  }
		  }
	  }
