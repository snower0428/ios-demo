...
//  }
    ...
    if (i++%5 == 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kFailedToDecodeImageNotification object:self];
    }
}
...
- (ArrayRef<char>) prepareSubset {
  ...
  NSLog(@"released context");  
    ...
    if (i%30 == 0) {
        ...
        NSData *data = UIImagePNGRepresentation(self.subsetImage);
        if (data == nil) {
            data = UIImageJPEGRepresentation(self.subsetImage, 1.0);
        }
