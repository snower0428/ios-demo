...
+ (NSInteger)nextLunarSpecialIntervalDaySinceDate:(NSDate *)date {
	...
	count++;
	NSDate *nextDate = [[NSDate alloc] initWithTimeInterval:timeIntervalDay sinceDate:curDayDate];
	specialDay = [self getLunarSpecialDay:nextDate];
	while (nil==specialDay && count<=20) {
		count++;
		[nextDate release];
		nextDate = [[NSDate alloc] initWithTimeInterval:timeIntervalDay sinceDate:nextDate];
		specialDay = [self getLunarSpecialDay:nextDate];
	}
	if (count > 20) {
		count = -1;
